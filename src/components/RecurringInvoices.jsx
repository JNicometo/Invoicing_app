...
function RecurringInvoices() {
  ...
  useEffect(() => {
    filterInvoices();
  }, [searchTerm, recurringInvoices]);

  const loadData = async () => {
    try {
      setLoading(true);
      const [invoicesData, clientsData, savedItemsData, settingsData] = await Promise.all([
        getAllRecurringInvoices(),
        getAllClients(),
        getAllSavedItems(),
        getSettings()
  ...
  const handleToggleActive = async (invoice) => {
    try {
      await updateRecurringInvoice(
        invoice.id,
        { ...invoice, active: invoice.active ? 0 : 1 },
        invoice.items
      );
      await loadData();
    } catch (error) {
      alert('Error updating recurring invoice: ' + error.message);
    }
  };
